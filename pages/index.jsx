...
function RecipeCard({ recipe, onClick, onDelete }) {
  ...
  return (
    <div onClick={onClick}
      ...
      style={{
        background:"#17171f", border:`1.5px solid ${hov?"#e8825a44":"#25253a"}`,
                  ...
                  <div style={{ background:"#1a1a2a", borderRadius:12, padding:"6px 14px", border:"1px solid #25253a" }}>
                    {recipe.ingredients.map((ing,i)=>(
                      <div key={i} style={{
                        ...
                        borderBottom:i<recipe.ingredients.length-1?"1px solid #25253a":"none", fontSize:13,
                      }}>
            ...
            <>
              {(recipe.comments||[]).length===0 ? (
                <div style={{ textAlign:"center", padding:"30px 0", color:"#444" }}>
                  ...
                  <div style={{ fontSize:13, lineHeight:1.9 }}>まだ記録がありません<br/>作ってみたら写真を投稿しよう！</div>
                </div>
              ) : (
                    ...
                    <div key={c.id} style={{
                      ...
                      {c.photo && (
                        <img src={c.photo} alt="投稿写真" style={{
                          width:"100%", borderRadius:10, marginBottom:c.text?9:0, maxHeight:240, objectFit:"cover",
                        }}/>
                      )}
              ...
              <div style={{ background:"#1e1e2e", borderRadius:16, padding:"14px", border:"1px solid #25253a" }}>
                ...
                <textarea value={commentText} onChange={e=>setCommentText(e.target.value)}
                  ...
                  style={{
                    ...
                    fontSize:13, outline:"none", resize:"none", boxSizing:"border-box", lineHeight:1.6, marginBottom:10,
                  }}/>
                {photoLoading && <div style={{ color:"#e8825a", fontSize:12, marginBottom:8, textAlign:"center" }}>写真を読み込み中...</div>}
                ...
                <div style={{ display:"flex", gap:8 }}>
                  <button onClick={()=>photoRef.current?.click()} disabled={photoLoading} style={{
                    ...
                    background:"#0f0f1a", color:photoLoading?"#444":"#777", fontSize:12,
                    cursor:photoLoading?"default":"pointer", whiteSpace:"nowrap",
    ...
    <div style={{
      ...
      <div style={{ maxWidth:360, width:"100%", textAlign:"center" }}>
        ...
        <input value={nameInput} onChange={e=>setNameInput(e.target.value)}
          onKeyDown={e=>{if(e.key==="Enter"&&nameInput.trim()){localStorage.setItem("rs-name",nameInput.trim());setUserName(nameInput.trim());}}}
        ...
        <button onClick={()=>{if(nameInput.trim()){localStorage.setItem("rs-name",nameInput.trim());setUserName(nameInput.trim());}}} style={{
          width:"100%", padding:"14px", borderRadius:14, border:"none",
          background:nameInput.trim()?"linear-gradient(135deg,#e8825a,#c8603a)":"#25253a",
          color:"#fff", fontSize:15, fontWeight:700, cursor:nameInput.trim()?"pointer":"default",
        }}>はじめる →</button>
      </div>
    </div>
  );
    ...
    <div style={{ minHeight:"100vh", background:"#0f0f14" }}>
          ...
          {allTags.length>0 && (
            <div style={{ display:"flex", gap:5, overflowX:"auto", paddingBottom:2 }}>
              <button onClick={()=>setActiveTag("")} style={{
                background:!activeTag?"#e8825a":"#1a1a28", color:!activeTag?"#fff":"#555",
              ...
              {allTags.map((t,i)=>{
                ...
                return (
                  <button key={i} onClick={()=>setActiveTag(activeTag===t?"":t)} style={{
                    background:activeTag===t?c+"22":"#1a1a28", color:activeTag===t?c:"#555",
      ...
      {view==="detail" && selected && (
        <RecipeDetail recipe={selected}
          ...
          onUpdate={handleUpdate} userName={userName}/>
      )}
      <Toast msg={toast} onClear={()=>setToast("")}/>
    </div>
  );
